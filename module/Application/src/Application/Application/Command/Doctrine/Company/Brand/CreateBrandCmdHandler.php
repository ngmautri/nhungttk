 ...
 *
 */
class CreateBrandCmdHandler extends AbstractCommandHandler
{
    ...
    public function run(CommandInterface $cmd)
    {
        ...
        try {
            ...
            $rootEntity = $companyEntity->createBrandFrom($snapshot, $options, $sharedService);

            $snapshot->id = $rootEntity->getId();
            $snapshot->uuid = $rootEntity->getUuid();
            $this->setOutput($snapshot); // important;

            $m = sprintf("[OK] Brand #%s created!", $snapshot->getId());
            $cmd->addSuccess($m);
