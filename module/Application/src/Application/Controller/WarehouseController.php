 ...
 *
 */
class WarehouseController extends EntityCRUDController
{
    ...
    {
        $this->baseUrl = '/application/warehouse';
    }
    ...
    public function addMemberAction()
    {
        ...
        if ($prg instanceof \Zend\Http\PhpEnvironment\Response) {
            ...
            $snapshot->setParentCode($parentCode);
            $snapshot->setWarehouse($rootId);
            $form->bind($snapshot);
        ...
        $notification = null;
        $rootEntityId = $data['warehouse'];
        $parentCode = $data['parentCode'];
        $form->setRedirectUrl(\sprintf("/application/warehouse/view?id=%s", $rootEntityId));
        ...
        $cmdHandler = new CreateLocationCmdHandler();
        $cmdHandlerDecorator = new TransactionalCommandHandler($cmdHandler);
        $cmd = new GenericCommand($this->getDoctrineEM(), $data, $options, $cmdHandlerDecorator, $this->getEventBusService());
        $cmd->setLogger($this->getLogger());
        ...
        if ($notification->hasErrors()) {
            ...
            $snapshot = new LocationSnapshot();
            $snapshot->setParentCode($parentCode);
            $snapshot->setWarehouse($rootEntityId);
            $form->bind($snapshot);
        ...
        $redirectUrl = \sprintf("/application/warehouse/view?id=%s", $rootEntityId);
    ...
    public function updateMemberAction()
    {
        ...
        if ($prg instanceof \Zend\Http\PhpEnvironment\Response) {
            ...
            $viewModel = new ViewModel(array(
                'errors' => null,
                'parentCode' => $snapshot->getParentCode(),
                'redirectUrl' => null,
                'version' => null,
                'nmtPlugin' => $nmtPlugin,
                'form_action' => $form_action,
    ...
    public function listAction()
    {
        ...
        $whCollection = $results->getLazyWarehouseCollection();
    ...
    public function girdAction()
    {
        ...
        $chart = $results->getLazyWarehouseCollection();
     ...
     */
    public function locationGirdAction()
    {
        ...
        /**
         *
         * @var GenericWarehouse $entity ;
         */
        $entity = $this->getEntityService()->getRootEntityById($rootId);
        $locCollection = $entity->getLazyLocationCollection();

        $total_records = $locCollection->count();
        $data = $locCollection;
        ...
        $data = new ArrayCollection($locCollection->slice($offset, $limit));
