...
class PorterStemmer implements StemmerInterface
{
    ...
    private static function doPartA($word)
    {
        if (substr($word, - 1) == 's') {
            ...
            self::replace($word, 'sses', 'ss') || self::replace($word, 'ies', 'i') || self::replace($word, 'ss', 'ss') || self::replace($word, 's', '');
        }
        return $word;
    }
    ...
    private static function doPartB($word)
    {
        if (substr($word, - 2, 1) != 'e' || ! self::replace($word, 'eed', 'ee', 0)) {
            ...
            // ing and ed
            if (preg_match("#$v+#", substr($word, 0, - 3)) && self::replace($word, 'ing', '') || preg_match("#$v+#", substr($word, 0, - 2)) && self::replace($word, 'ed', '')) {
    ...
    private static function step2($word)
    {
        switch (substr($word, - 2, 1)) {
            case 'a':
                self::replace($word, 'ational', 'ate', 0) || self::replace($word, 'tional', 'tion', 0);
                break;

            case 'c':
                self::replace($word, 'enci', 'ence', 0) || self::replace($word, 'anci', 'ance', 0);
                break;
            ...
            case 'l':
                self::replace($word, 'entli', 'ent', 0) || self::replace($word, 'ousli', 'ous', 0) || self::replace($word, 'alli', 'al', 0) || self::replace($word, 'bli', 'ble', 0) || self::replace($word, 'eli', 'e', 0);
                break;

            case 'o':
                self::replace($word, 'ization', 'ize', 0) || self::replace($word, 'ation', 'ate', 0) || self::replace($word, 'ator', 'ate', 0);
                break;

            case 's':
                self::replace($word, 'iveness', 'ive', 0) || self::replace($word, 'fulness', 'ful', 0) || self::replace($word, 'ousness', 'ous', 0) || self::replace($word, 'alism', 'al', 0);
                break;

            case 't':
                self::replace($word, 'biliti', 'ble', 0) || self::replace($word, 'aliti', 'al', 0) || self::replace($word, 'iviti', 'ive', 0);
                break;
        }
    ...
    private static function step3($word)
    {
        switch (substr($word, - 2, 1)) {
            ...
            case 't':
                self::replace($word, 'icate', 'ic', 0) || self::replace($word, 'iciti', 'ic', 0);
                break;
    ...
    private static function step4($word)
    {
        switch (substr($word, - 2, 1)) {
            ...
            case 'c':
                self::replace($word, 'ance', '', 1) || self::replace($word, 'ence', '', 1);
                break;
            ...
            case 'l':
                self::replace($word, 'able', '', 1) || self::replace($word, 'ible', '', 1);
                break;

            case 'n':
                self::replace($word, 'ant', '', 1) || self::replace($word, 'ement', '', 1) || self::replace($word, 'ment', '', 1) || self::replace($word, 'ent', '', 1);
                break;
            ...
            case 't':
                self::replace($word, 'ate', '', 1) || self::replace($word, 'iti', '', 1);
                break;
