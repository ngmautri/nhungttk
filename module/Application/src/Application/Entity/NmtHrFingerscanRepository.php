...
class NmtHrFingerscanRepository extends EntityRepository
{
    ...
    public function getFingerscan($employee_code = null, $day, $month, $year)
    {
        ...
        $s = "";
        for ($i = 1; $i <= 31; $i ++) {
            $s = $s . "sum(case when nmt_hr_fingerscan.dd ='" . $i . "-" . $month . "-" . $year . "' then hour(nmt_hr_fingerscan.clock_in) else 0 end ) as hh_in_" . $i . ", ";

            $s = $s . "sum(case when nmt_hr_fingerscan.dd ='" . $i . "-" . $month . "-" . $year . "' then minute(nmt_hr_fingerscan.clock_in) else 0 end ) as mm_in_" . $i . ", ";

            $s = $s . "sum(case when nmt_hr_fingerscan.dd ='" . $i . "-" . $month . "-" . $year . "' then hour(nmt_hr_fingerscan.clock_out) else 0 end ) as hh_out_" . $i . ", ";

            $s = $s . "sum(case when nmt_hr_fingerscan.dd ='" . $i . "-" . $month . "-" . $year . "' then minute(nmt_hr_fingerscan.clock_out) else 0 end ) as mm_out_" . $i . ", ";
        }
