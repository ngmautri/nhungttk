...
class DepartmentMemberTable
{
    ...
    public function getMembersByCatId($id)
    {
        ...
        $select->from(array(
            't1' => 'mla_sparepart_cats_members'
        ));
        $select->join(array(
            't2' => 'mla_spareparts'
        ), 't2.id = t1.sparepart_id', '*', \Zend\Db\Sql\Select::JOIN_INNER);
        $select->where($where);
...
WHERE lt1.sparepart_cat_id ='" . $id . "' limit " . $limit . ' offset ' . $offset;
    ...
    public function getNoneMembersOfDepartment($id)
    {
        ...
        $select->join(array(
            't2' => 'mla_users'
        ), 't2.id = t1.user_id', array(), \Zend\Db\Sql\Select::JOIN_INNER);
        $select->where($where);
    ...
    {
        $sql = "SELECT t1.*, t2.firstname, t2.lastname, t2.email, t3.name as department_name from mla_departments_members as t1
