...
EOD;
	...
	public function savePOAsPdf(
			...
			$detail
				...
				$pdf->SetCreator ( PDF_CREATOR );
				$pdf->SetAuthor ( 'Mascot Laos ' );
				$pdf->SetTitle ( 'Purchase Order ' . $pr_number);
				$pdf->SetSubject ( 'TCPDF Tutorial' );
				$pdf->SetKeywords ( 'TCPDF, PDF, example, test, guide' );
				...
				$pdf->Write(0, 'Purchase Order', '', 0, 'C', true, 0, false, false, 0);
				$pdf->Ln(2);
				$pdf->SetFont('helvetica', 'I', 10);
				$pdf->Write(0, 'Ref.: PR-' .$pr_number , '', 0, 'C', true, 0, false, false, 0);
				$pdf->Write(0, 'Date: ' .$date , '', 0, 'C', true, 0, false, false, 0);
...
<td style="text-align: center">Checked by HOD
</td>
<td style="text-align: center">Concured by Finance Manager</td>
<td style="text-align: center">Approved by Managing Director</td>
</tr>
...
EOD;
				...
				//Close and output PDF document
				$pdf->Output('PO.'.$pr_auto_number.'.pdf', 'I');
	}
