...
class EmployeeSearchService extends AbstractService {
	...
	public function createIndex() {
		...
		if (count ( $rows ) > 0) {
			foreach ( $rows as $row ) {
				...
				$doc->addField ( Field::Keyword ( 'employee_code_keyword', $row->employee_code ) );
	...
	/**
	 */
	public function updateIndexOfEmployee($employee_code) {
		$index = Lucene::open ( ROOT . DIRECTORY_SEPARATOR . $this->index_path );
		Analyzer::setDefault ( new CaseInsensitive () );
		
		$hits = $index->find ( 'employee_code_keyword:' . $employee_code );
			...
			foreach ( $hits as $hit ) {
				...
				$row = $this->employeeTable->getEmployeeByCode ( $employee_code );
				if ($row != null) {
					$doc = new Document ();
					$doc->addField ( Field::UnIndexed ( 'employee_id', $row->id ) );
			...
			return "Updating OK for ID " . $employee_code;
		} else {
			return "NOT OK";
		}
	}
