 ...
 *
 */
class ItemSearchIndexImpl_V01 extends AbstractService implements ItemSearchIndexInterface
{
    ...
    private function _updateIndexFromSnapshot(SearchIndexInterface $indexer, ItemSnapshot $snapshot)
    {
        ...
        // step 1: create index doc for item

        // step 2: create index doc for serial number, if any

        // step 3: create index doc for variant, if any
    }
    ...
    private function __createPartForVariant(Document $doc, ItemSnapshot $snapshot, GenericVariant $variant = null)
    {
        if ($variant != null) {
            ...
            $doc->addField(Field::UnIndexed('variantId', null));
            $doc->addField(Field::keyword('variantCode', null));
            $doc->addField(Field::keyword('variantSysNumber', null));
            $doc->addField(Field::text('variantName', null));
            $doc->addField(Field::text('variantFullName', null));
            $doc->addField(Field::keyword('variantSysNumber', null));
        }
    ...
    private function __createPartForPicture(Document $doc, ItemSnapshot $snapshot, NmtInventoryItemSerial $sn = null)
    {}
