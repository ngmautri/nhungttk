 ...
 *        
 */
class GIforTransferToWH extends \Inventory\Model\AbstractTransactionStrategy
{
    ...
    public function validateHeader($entity, $data, $u, $isNew, $isPosting)
    {
        ...
        if (isset($data['target_wh_id'])) {
            ...
            $errors[] = $this->controllerPlugin->translate('No input given "target_warehouse_id"');
        }
        ...
        $target_wh = $this->getContextService()
            ->getDoctrineEM()
        ...
        if ($target_wh == null) {
            $errors[] = $this->getContextService()
                ->getControllerPlugin()
                ->translate('Target Warehouse is required');
        } else {
            $n_check ++;
            $entity->setTargetWarehouse($target_wh);
        }
        ...
        if ($n_check == 2 and $source_wh == $target_wh) {
            $errors[] = $this->getContextService()
                ->getControllerPlugin()
                ->translate('Warehouses are the same. Do you want to transfer location in this warehouse?');
        }
