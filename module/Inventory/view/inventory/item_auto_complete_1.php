...
if ($dto != null) {
    ...
    $format = '$("#issue_for_url").text(\'%s\'); $("#issue_for_detail").show();';
...
$( "#issueForName" ).autocomplete({
    source: "/inventory/item-search/auto-complete2",
    minLength: 2,
    select: function( event, ui ) {

         // set pr row empty
        $( "#issueFor" ).val(ui.item.hit.id);

        $('#issue_for_url').text('/inventory/item/show1?tab_idx=7&entity_id='+ui.item.hit.id+'&token='+ui.item.hit.token);
...
function showSelectedIssueForItem(){
   	var url=  $('#issue_for_url').text();
