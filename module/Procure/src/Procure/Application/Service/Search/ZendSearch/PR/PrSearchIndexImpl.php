...
class PrSearchIndexImpl extends AbstractService implements PrSearchIndexInterface
{
    ...
    public function createIndex($rows)
    {
        ...
        try {
            ...
            $index = Lucene::create(getcwd() . PrSearch::INDEX_PATH);
            Analyzer::setDefault(new CaseInsensitive());
            ...
            $message = \sprintf('Failed! %s - %s', $e->getTraceAsString(), $row->getId() . $row->getItemName1());
            $indexResult->setMessage($message);
        }
    ...
    private function _createDoc(\ZendSearch\Lucene\SearchIndexInterface $index, GenericRow $row)
    {
        ...
        if (! $row instanceof PRRow) {
            throw new \InvalidArgumentException("PrRow empty");
        }
    ...
    private function getIndexer()
    {
        $indexer = null;
        try {
            $indexer = Lucene::open(getcwd() . PrSearch::INDEX_PATH);
        } catch (RuntimeException $e) {
            $indexer = Lucene::create(getcwd() . PrSearch::INDEX_PATH);
        }
