...
class POController extends AbstractActionController {
	...
	public function listAction() {
		...
		$po_items = $this->poItemTable->getPOItems($balance, $department_id,$vendor_id,0, 0 );
		$totalResults = count ( $po_items );
	
		$paginator = null;
		if ($totalResults > $resultsPerPage) {
			$paginator = new Paginator ( $totalResults, $page, $resultsPerPage );
			$po_items = $this->poItemTable->getPOItems ($balance,$department_id,$vendor_id, ($paginator->maxInPage - $paginator->minInPage) + 1, $paginator->minInPage - 1 );
		}
