...
abstract class AbstractCategory {
	...
	public function generateJSTree($root) {
		...
		//inorder travesal	
		if (count ( $children ) > 0) {
			$this->jsTree = $this->jsTree . '<li data-jstree=\'{ "opened" : true}\'> ' . ucwords($tree['instance']->name) . '('.count ( $children ).")\n";
			...
			foreach ( $children as $key => $value ) {
				...
				if (count ( $value ['children'] ) > 0) {
					...
					$this->jsTree = $this->jsTree . '<li data-jstree=\'{}\'>' . $value ['instance']->name . ' </li>' . "\n";
					$this->generateJSTree ($key);
				}
			}
			$this->jsTree = $this->jsTree . '</ul>' . "\n";
