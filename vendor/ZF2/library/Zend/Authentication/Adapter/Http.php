...
class Http implements AdapterInterface {
	 ...
	 * My intention is to support both 'auth' and
	 * 'auth-int', but 'auth-int' won't make it into the first version.
	 *
	 * @var array
	 */
	protected $supportedQops = array (
			'auth' 
	);
	...
	public function __construct(array $config) {
		...
		if (empty ( $this->acceptSchemes )) {
			throw new Exception\InvalidArgumentException ( sprintf ( 'No supported schemes given in "accept_schemes". Valid values: %s', implode ( ', ', $this->supportedSchemes ) ) );
		}
		...
		if (empty ( $config ['realm'] ) || ! ctype_print ( $config ['realm'] ) || strpos ( $config ['realm'], ':' ) !== false || strpos ( $config ['realm'], '"' ) !== false) {
			throw new Exception\InvalidArgumentException ( 'Config key \'realm\' is required, and must contain only printable characters,' . 'excluding quotation marks and colons' );
		} else {
		...
		if (in_array ( 'digest', $this->acceptSchemes )) {
			if (empty ( $config ['digest_domains'] ) || ! ctype_print ( $config ['digest_domains'] ) || strpos ( $config ['digest_domains'], '"' ) !== false) {
				throw new Exception\InvalidArgumentException ( 'Config key \'digest_domains\' is required, and must contain ' . 'only printable characters, excluding quotation marks' );
			} else {
				$this->domains = $config ['digest_domains'];
			}
			
			if (empty ( $config ['nonce_timeout'] ) || ! is_numeric ( $config ['nonce_timeout'] )) {
				throw new Exception\InvalidArgumentException ( 'Config key \'nonce_timeout\' is required, and must be an integer' );
			} else {
	...
	public function authenticate() {
		...
		if (! in_array ( $clientScheme, $this->supportedSchemes )) {
			...
			return new Authentication\Result ( Authentication\Result::FAILURE_UNCATEGORIZED, array (), array (
	...
	protected function _challengeClient() {
		...
		return new Authentication\Result ( Authentication\Result::FAILURE_CREDENTIAL_INVALID, array (), array (
	...
	protected function _digestHeader() {
		$wwwauth = 'Digest realm="' . $this->realm . '", ' . 'domain="' . $this->domains . '", ' . 'nonce="' . $this->_calcNonce () . '", ' . ($this->useOpaque ? 'opaque="' . $this->_calcOpaque () . '", ' : '') . 'algorithm="' . $this->algo . '", ' . 'qop="' . implode ( ',', $this->supportedQops ) . '"';
	...
	protected function _basicAuth($header) {
		...
		if (empty ( $this->basicResolver )) {
			throw new Exception\RuntimeException ( 'A basicResolver object must be set before doing Basic ' . 'authentication' );
		}
		...
		if (! $result instanceof Authentication\Result && ! is_array ( $result ) && CryptUtils::compareStrings ( $result, $creds [1] )) {
			$identity = array (
					'username' => $creds [0],
					'realm' => $this->realm 
			);
	...
	protected function _parseDigestAuth($header) {
		...
		if ($this->useOpaque) {
			...
			if (! $this->ieNoOpaque && (32 != strlen ( $temp [1] ) || ! ctype_xdigit ( $temp [1] ))) {
				return false;
			}
