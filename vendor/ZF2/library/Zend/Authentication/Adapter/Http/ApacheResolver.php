...
class ApacheResolver implements ResolverInterface {
	...
	public function resolve($username, $realm, $password = null) {
		...
		if (! ctype_print ( $username ) || strpos ( $username, ':' ) !== false) {
			throw new Exception\InvalidArgumentException ( 'Username must consist only of printable characters, excluding the colon' );
		}
		
		if (! empty ( $realm ) && (! ctype_print ( $realm ) || strpos ( $realm, ':' ) !== false)) {
			throw new Exception\InvalidArgumentException ( 'Realm must consist only of printable characters, excluding the colon' );
		}
		...
		if (! isset ( $matchedHash )) {
			return new AuthResult ( AuthResult::FAILURE_IDENTITY_NOT_FOUND, null, array (
		...
		return new AuthResult ( AuthResult::FAILURE_CREDENTIAL_INVALID, null, array (
