...
abstract class Barcode {
	...
	public static function factory($barcode, $renderer = 'image', $barcodeConfig = array(), $rendererConfig = array(), $automaticRenderError = true) {
		...
		try {
			...
			if ($automaticRenderError && ! ($e instanceof Exception\RendererCreationException)) {
				$barcode = static::makeBarcode ( 'error', array (
						'text' => $e->getMessage () 
				) );
	...
	public static function makeBarcode($barcode, $barcodeConfig = array()) {
		...
		if (! is_array ( $barcodeConfig )) {
			throw new Exception\InvalidArgumentException ( 'Barcode parameters must be in an array or a Traversable object' );
		}
		...
		if (! is_string ( $barcode ) || empty ( $barcode )) {
			throw new Exception\InvalidArgumentException ( 'Barcode name must be specified in a string' );
		}
	...
	public static function makeRenderer($renderer = 'image', $rendererConfig = array()) {
		...
		if (! is_array ( $rendererConfig )) {
			throw new Exception\RendererCreationException ( 'Barcode parameters must be in an array or a Traversable object' );
		}
		...
		if (! is_string ( $renderer ) || empty ( $renderer )) {
			throw new Exception\RendererCreationException ( 'Renderer name must be specified in a string' );
		}
