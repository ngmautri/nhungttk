...
abstract class AbstractObject implements ObjectInterface {
	...
	public function setBarHeight($value) {
		if (intval ( $value ) <= 0) {
			throw new Exception\OutOfRangeException ( 'Bar height must be greater than 0' );
		}
	...
	public function setBarThinWidth($value) {
		if (intval ( $value ) <= 0) {
			throw new Exception\OutOfRangeException ( 'Bar width must be greater than 0' );
		}
	...
	public function setFactor($value) {
		if (floatval ( $value ) <= 0) {
			throw new Exception\OutOfRangeException ( 'Factor must be greater than 0' );
		}
	...
	public function setForeColor($value) {
		if (preg_match ( '`\#[0-9A-F]{6}`', $value )) {
			...
			throw new Exception\InvalidArgumentException ( 'Text color must be set as #[0-9A-F]{6}' );
		}
		return $this;
	}
	...
	public function setBackgroundColor($value) {
		if (preg_match ( '`\#[0-9A-F]{6}`', $value )) {
			...
			throw new Exception\InvalidArgumentException ( 'Background color must be set as #[0-9A-F]{6}' );
		}
		return $this;
	}
	...
	public function setFont($value) {
		if (is_int ( $value ) && $value >= 1 && $value <= 5) {
			if (! extension_loaded ( 'gd' )) {
				throw new Exception\ExtensionNotLoadedException ( 'GD extension is required to use numeric font' );
			}
			...
			throw new Exception\InvalidArgumentException ( sprintf ( 'Invalid font "%s" provided to setFont()', $value ) );
		}
		return $this;
	}
	...
	public function setFontSize($value) {
		...
		if (! is_numeric ( $value )) {
			throw new Exception\InvalidArgumentException ( 'Font size must be a numeric value' );
		}
	 ...
	 */
	protected function addText($text, $size, $position, $font, $color, $alignment = 'center', $orientation = 0) {
		if ($color === null) {
			$color = $this->foreColor;
		}
		$this->addInstruction ( array (
	...
	protected function checkText($value = null) {
		...
		if (! strlen ( $value )) {
			throw new Exception\RuntimeException ( 'A text must be provide to Barcode before drawing' );
		}
	...
	protected function checkRatio($min = 2, $max = 3) {
		...
		if (! ($ratio >= $min && $ratio <= $max)) {
			throw new Exception\OutOfRangeException ( sprintf ( 'Ratio thick/thin bar must be between %0.1f and %0.1f (actual %0.3f)', $min, $max, $ratio ) );
		}
	}
	...
	protected function checkFontAndOrientation() {
		if (is_numeric ( $this->font ) && $this->orientation != 0) {
			throw new Exception\RuntimeException ( 'Only drawing with TTF font allow orientation of the barcode.' );
		}
	}
	...
	protected function calculateWidth() {
		return ( int ) $this->withBorder + $this->calculateBarcodeWidth () + ( int ) $this->withBorder;
	}
	...
	protected function calculateHeight() {
		return ( int ) $this->withBorder * 2 + $this->calculateBarcodeHeight () + ( int ) $this->withBorder * 2;
	}
	...
	public function getHeight($recalculate = false) {
		if ($this->height === null || $recalculate) {
			$this->height = abs ( $this->calculateHeight () * cos ( $this->orientation / 180 * pi () ) ) + abs ( $this->calculateWidth () * sin ( $this->orientation / 180 * pi () ) );
		}
		return $this->height;
	}
	...
	public function getWidth($recalculate = false) {
		if ($this->width === null || $recalculate) {
			$this->width = abs ( $this->calculateWidth () * cos ( $this->orientation / 180 * pi () ) ) + abs ( $this->calculateHeight () * sin ( $this->orientation / 180 * pi () ) );
		}
		return $this->width;
	}
	...
	protected function rotate($x1, $y1) {
		$x2 = $x1 * cos ( $this->orientation / 180 * pi () ) - $y1 * sin ( $this->orientation / 180 * pi () ) + $this->getOffsetLeft ();
		$y2 = $y1 * cos ( $this->orientation / 180 * pi () ) + $x1 * sin ( $this->orientation / 180 * pi () ) + $this->getOffsetTop ();
		return array (
				intval ( $x2 ),
				intval ( $y2 ) 
		);
	...
	protected function drawBarcode() {
		...
		$point3 = $this->rotate ( $this->calculateWidth () - 1, $this->calculateHeight () - 1 );
		...
		foreach ( $barcodeTable as $bar ) {
			...
			if ($bar [0]) {
				...
				$point3 = $this->rotate ( $xpos + $width - 1, $ypos + $bar [3] * $barLength );
				$point4 = $this->rotate ( $xpos + $width - 1, $ypos + $bar [2] * $barLength );
				$this->addPolygon ( array (
						$point1,
						$point2,
						$point3,
		...
		if ($this->drawText) {
			...
			if ($this->stretchText) {
				...
				for($i = 0; $i < $textLength; $i ++) {
					...
					$this->addText ( $text {$i}, $this->fontSize * $this->factor, $this->rotate ( $leftPosition, ( int ) $this->withBorder * 2 + $this->factor * ($this->barHeight + $this->fontSize) + 1 ), $this->font, $this->foreColor, 'center', - $this->orientation );
				}
			} else {
				$this->addText ( $text, $this->fontSize * $this->factor, $this->rotate ( $this->calculateWidth () / 2, ( int ) $this->withBorder * 2 + $this->factor * ($this->barHeight + $this->fontSize) + 1 ), $this->font, $this->foreColor, 'center', - $this->orientation );
			}
		}
	}
