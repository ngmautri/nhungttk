...
class Code128 extends AbstractObject {
	...
	protected function convertToBarcodeChars($string) {
		...
		for($pos = 0; $pos < strlen ( $string ); $pos ++) {
			...
			if (static::_isDigit ( $string, $pos, 4 ) && $currentCharset != 'C' || static::_isDigit ( $string, $pos, 2 ) && $currentCharset == 'C') {
				/**
			...
			} elseif (in_array ( $char, $this->charSets ['B'] ) && $currentCharset != 'B' && ! (in_array ( $char, $this->charSets ['A'] ) && $currentCharset == 'A')) {
				/**
				...
				$result [] = $code;
				$currentCharset = 'B';
			} elseif (array_key_exists ( $char, $this->charSets ['A'] ) && $currentCharset != 'A' && ! (array_key_exists ( $char, $this->charSets ['B'] ) && $currentCharset == 'B')) {
				/**
