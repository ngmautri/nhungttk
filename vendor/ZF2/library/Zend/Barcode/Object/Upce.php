	...
	protected function drawText() {
		if ($this->drawText) {
			...
			for($i = 0; $i < $this->barcodeLength; $i ++) {
				...
				$this->addText ( $text {$i}, $fontSize * $this->factor, $this->rotate ( $leftPosition, ( int ) $this->withBorder * 2 + $this->factor * ($this->barHeight + $fontSize) + 1 ), $this->font, $this->foreColor, 'left', - $this->orientation );
				switch ($i) {
					case 0 :
						$factor = 3;
						break;
