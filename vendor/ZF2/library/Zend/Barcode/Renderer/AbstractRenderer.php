...
abstract class AbstractRenderer implements RendererInterface {
	...
	public function __construct($options = null) {
		...
		$this->type = strtolower ( substr ( get_class ( $this ), strlen ( $this->rendererNamespace ) + 1 ) );
	}
	...
	public function setTopOffset($value) {
		if (! is_numeric ( $value ) || intval ( $value ) < 0) {
			throw new Exception\OutOfRangeException ( 'Vertical position must be greater than or equals 0' );
		}
	...
	public function setLeftOffset($value) {
		if (! is_numeric ( $value ) || intval ( $value ) < 0) {
			throw new Exception\OutOfRangeException ( 'Horizontal position must be greater than or equals 0' );
		}
	...
	public function setHorizontalPosition($value) {
		if (! in_array ( $value, array (
			...
			throw new Exception\UnexpectedValueException ( "Invalid barcode position provided must be 'left', 'center' or 'right'" );
		}
	...
	public function setVerticalPosition($value) {
		if (! in_array ( $value, array (
			...
			throw new Exception\UnexpectedValueException ( "Invalid barcode position provided must be 'top', 'middle' or 'bottom'" );
		}
	...
	public function setModuleSize($value) {
		if (! is_numeric ( $value ) || floatval ( $value ) <= 0) {
			throw new Exception\OutOfRangeException ( 'Float size must be greater than 0' );
		}
	...
	protected function checkBarcodeObject() {
		if ($this->barcode === null) {
			throw new Exception\RuntimeException ( 'No barcode object provided' );
		}
	}
	...
	public function draw() {
		try {
			...
			if ($this->automaticRenderError && ! ($e instanceof BarcodeException\RendererCreationException)) {
				$barcode = Barcode::makeBarcode ( 'error', array (
						'text' => $e->getMessage () 
				) );
	 ...
	 */
	abstract protected function drawText($text, $size, $position, $font, $color, $alignment = 'center', $orientation = 0);
}
