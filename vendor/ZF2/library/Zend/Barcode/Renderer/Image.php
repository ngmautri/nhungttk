...
class Image extends AbstractRenderer {
	...
	public function setHeight($value) {
		if (! is_numeric ( $value ) || intval ( $value ) < 0) {
			throw new Exception\OutOfRangeException ( 'Image height must be greater than or equals 0' );
		}
	...
	public function setWidth($value) {
		if (! is_numeric ( $value ) || intval ( $value ) < 0) {
			throw new Exception\OutOfRangeException ( 'Image width must be greater than or equals 0' );
		}
	...
	public function setResource($image) {
		if (gettype ( $image ) != 'resource' || get_resource_type ( $image ) != 'gd') {
			throw new Exception\InvalidArgumentException ( 'Invalid image resource provided to setResource()' );
		}
		$this->resource = $image;
		return $this;
	}
	...
	public function setImageType($value) {
		...
		if (! in_array ( $value, $this->allowedImageType )) {
			throw new Exception\InvalidArgumentException ( sprintf ( 'Invalid type "%s" provided to setImageType()', $value ) );
		}
	...
	protected function initRenderer() {
		...
		if ($this->resource !== null) {
			...
			$this->imageBackgroundColor = imagecolorallocate ( $this->resource, ($backgroundColor & 0xFF0000) >> 16, ($backgroundColor & 0x00FF00) >> 8, $backgroundColor & 0x0000FF );
			$this->imageForeColor = imagecolorallocate ( $this->resource, ($foreColor & 0xFF0000) >> 16, ($foreColor & 0x00FF00) >> 8, $foreColor & 0x0000FF );
		} else {
			$width = $barcodeWidth;
		...
		imagefilledrectangle ( $this->resource, $this->leftOffset, $this->topOffset, $this->leftOffset + $barcodeWidth - 1, $this->topOffset + $barcodeHeight - 1, $this->imageBackgroundColor );
	}
	...
	protected function checkDimensions() {
		if ($this->resource !== null) {
			if (imagesy ( $this->resource ) < $this->barcode->getHeight ( true )) {
				throw new Exception\RuntimeException ( 'Barcode is define outside the image (height)' );
			}
		} else {
			if ($this->userHeight) {
				$height = $this->barcode->getHeight ( true );
				if ($this->userHeight < $height) {
					throw new Exception\RuntimeException ( sprintf ( "Barcode is define outside the image (calculated: '%d', provided: '%d')", $height, $this->userHeight ) );
				}
			}
		}
		if ($this->resource !== null) {
			if (imagesx ( $this->resource ) < $this->barcode->getWidth ( true )) {
				throw new Exception\RuntimeException ( 'Barcode is define outside the image (width)' );
			}
		} else {
			if ($this->userWidth) {
				$width = $this->barcode->getWidth ( true );
				if ($this->userWidth < $width) {
					throw new Exception\RuntimeException ( sprintf ( "Barcode is define outside the image (calculated: '%d', provided: '%d')", $width, $this->userWidth ) );
				}
			}
		}
	}
	...
	protected function drawPolygon($points, $color, $filled = true) {
		...
		$allocatedColor = imagecolorallocate ( $this->resource, ($color & 0xFF0000) >> 16, ($color & 0x00FF00) >> 8, $color & 0x0000FF );
	...
	protected function drawText($text, $size, $position, $font, $color, $alignment = 'center', $orientation = 0) {
		...
		if (is_numeric ( $font )) {
			...
			imagettftext ( $this->resource, $size, $orientation, $position [0] - ($width * cos ( pi () * $orientation / 180 )), $position [1] + ($width * sin ( pi () * $orientation / 180 )), $allocatedColor, $font, $text );
		}
	}
}
