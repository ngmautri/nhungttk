...
class Svg extends AbstractRenderer {
	...
	public function setHeight($value) {
		if (! is_numeric ( $value ) || intval ( $value ) < 0) {
			throw new Exception\OutOfRangeException ( 'Svg height must be greater than or equals 0' );
		}
	...
	public function setWidth($value) {
		if (! is_numeric ( $value ) || intval ( $value ) < 0) {
			throw new Exception\OutOfRangeException ( 'Svg width must be greater than or equals 0' );
		}
	...
	protected function initRenderer() {
		...
		if ($this->resource === null) {
			...
			$this->appendRootElement ( 'title', array (), "Barcode " . strtoupper ( $this->barcode->getType () ) . " " . $this->barcode->getText () );
		} else {
		...
		$this->appendRootElement ( 'rect', array (
				'x' => $this->leftOffset,
				'y' => $this->topOffset,
	...
	protected function drawText($text, $size, $position, $font, $color, $alignment = 'center', $orientation = 0) {
		...
		$attributes ['transform'] = 'rotate(' . (- $orientation) . ', ' . ($position [0] + $this->leftOffset) . ', ' . ($position [1] + $this->topOffset) . ')';
