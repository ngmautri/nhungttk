...
class ObjectCache extends CallbackCache {
		...
		switch ($method) {
			case '__set' :
				...
				if (method_exists ( $object, '__get' )) {
					$removeKeys [] = $this->generateKey ( '__get', array (
							$property 
					) );
				}
				if (method_exists ( $object, '__isset' )) {
					$removeKeys [] = $this->generateKey ( '__isset', array (
							$property 
					) );
				}
				if ($removeKeys) {
	...
	public function generateKey($method, array $args = array()) {
		return $this->generateCallbackKey ( array (
				$this->getOptions ()->getObject (),
				$method 
		), $args );
	}
	...
	public function __isset($name) {
		return $this->call ( '__isset', array (
				$name 
		) );
	}
	...
	public function __unset($name) {
		return $this->call ( '__unset', array (
				$name 
		) );
	}
