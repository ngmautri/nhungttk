...
class Apc extends AbstractAdapter implements AvailableSpaceCapableInterface, ClearByNamespaceInterface, ClearByPrefixInterface, FlushableInterface, IterableInterface, TotalSpaceCapableInterface {
	/**
	...
	protected function internalSetItem(& $normalizedKey, & $value) {
		...
		if (! apc_store ( $internalKey, $value, $ttl )) {
			...
			throw new Exception\RuntimeException ( "apc_store('{$internalKey}', <{$type}>, {$ttl}) failed" );
		}
	...
	protected function internalAddItem(& $normalizedKey, & $value) {
		...
		if (! apc_add ( $internalKey, $value, $ttl )) {
			...
			$type = is_object ( $value ) ? get_class ( $value ) : gettype ( $value );
			throw new Exception\RuntimeException ( "apc_add('{$internalKey}', <{$type}>, {$ttl}) failed" );
		}
	...
	protected function internalGetCapabilities() {
		if ($this->capabilities === null) {
			$marker = new stdClass ();
			$capabilities = new Capabilities ( $this, $marker, array (
	...
	protected function normalizeMetadata(array & $metadata) {
		...
		unset ( $metadata ['key'], $metadata ['creation_time'], $metadata ['access_time'], $metadata ['deletion_time'], $metadata ['mem_size'], $metadata ['num_hits'] );
	}
}
