...
class Image extends AbstractWord {
	...
	protected function generateImage($id, $word) {
		...
		if (empty ( $this->startImage )) {
			...
			if (! $img || $error) {
				throw new Exception\ImageNotLoadableException ( "Can not load start image '{$this->startImage}'", 0, $error );
			}
			$w = imagesx ( $img );
			$h = imagesy ( $img );
		}
		...
		for($x = 0; $x < $w; $x ++) {
			for($y = 0; $y < $h; $y ++) {
				...
				if ($color == 255 && $colorX == 255 && $colorY == 255 && $colorXY == 255) {
					...
					$newcolor = $color * $fracX1 * $fracY1 + $colorX * $fracX * $fracY1 + $colorY * $fracX1 * $fracY + $colorXY * $fracX * $fracY;
				}
