...
class PropertyGenerator extends AbstractMemberGenerator {
	...
	public static function fromArray(array $array) {
		...
		foreach ( $array as $name => $value ) {
			// normalize key
			switch (strtolower ( str_replace ( array (
					'.',
					'-',
					'_' 
			), '', $name ) )) {
	...
	public function generate() {
		...
		if ($this->isConst ()) {
			...
			$output .= $this->indentation . 'const ' . $name . ' = ' . (($defaultValue !== null) ? $defaultValue->generate () : 'null;');
		} else {
			$output .= $this->indentation . $this->getVisibility () . (($this->isStatic ()) ? ' static' : '') . ' $' . $name . ' = ' . (($defaultValue !== null) ? $defaultValue->generate () : 'null;');
		}
