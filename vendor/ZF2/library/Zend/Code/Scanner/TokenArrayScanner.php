...
class TokenArrayScanner implements ScannerInterface {
	...
	public function getClass($name) {
		...
		return new ClassScanner ( array_slice ( $this->tokens, $info ['tokenStart'], ($info ['tokenEnd'] - $info ['tokenStart'] + 1) ), // zero indexed array
	...
	protected function scan() {
		...
		switch ($tokenType) {
			...
			case T_INTERFACE :
				...
				// process the name
				if ($infos [$infoIndex] ['shortName'] == '' && (($tokenType === T_CLASS || $tokenType === T_INTERFACE) && $infos [$infoIndex] ['type'] === 'class' || ($tokenType === T_FUNCTION && $infos [$infoIndex] ['type'] === 'function'))) {
