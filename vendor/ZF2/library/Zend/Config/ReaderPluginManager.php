...
class ReaderPluginManager extends AbstractPluginManager {
	...
	public function validatePlugin($plugin) {
		...
		throw new Exception\InvalidArgumentException ( sprintf ( 'Plugin of type %s is invalid; must implement %s\Reader\ReaderInterface', (is_object ( $plugin ) ? get_class ( $plugin ) : gettype ( $plugin )), __NAMESPACE__ ) );
	}
}
