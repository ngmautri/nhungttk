...
class Posix extends AbstractAdapter {
	...
	public function colorize($string, $color = null, $bgColor = null) {
		...
		return ($color !== null ? "\x1b[" . $color . 'm' : '') . ($bgColor !== null ? "\x1b[" . $bgColor . 'm' : '') . $string . "\x1b[22;39m\x1b[0;49m";
	}
	...
	protected function getColorCode($color, $type = 'fg') {
		...
		if ($color !== null) {
			if (! isset ( static::$ansiColorMap [$type] [$color] )) {
				throw new Exception\BadMethodCallException ( sprintf ( 'Unknown color "%s". Please use one of the Zend\Console\ColorInterface constants or use Zend\Console\Color\Xterm256::calculate', $color ) );
			}
