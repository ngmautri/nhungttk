...
class Windows extends Virtual {
	...
	protected function runProbeCommand() {
		exec ( 'echo $size = $Host.ui.rawui.windowsize; write $($size.width) $($size.height) | powershell -NonInteractive -NoProfile -NoLogo -OutputFormat Text -Command -', $output, $return );
	...
	public function readChar($mask = null) {
		...
		if ($mask === null) {
			exec ( 'powershell -NonInteractive -NoProfile -NoLogo -OutputFormat Text -Command "' . 'while ($Host.UI.RawUI.KeyAvailable) {$key = $Host.UI.RawUI.ReadKey(\'NoEcho,IncludeKeyDown\');}' . 'write $key.VirtualKeyCode;' . '"', $result, $return );
			...
			exec ( 'powershell -NonInteractive -NoProfile -NoLogo -OutputFormat Text -Command "' . '[int[]] $mask = ' . join ( ',', $asciiMask ) . ';' . 'do {' . '$key = $Host.UI.RawUI.ReadKey(\'NoEcho,IncludeKeyDown\').VirtualKeyCode;' . '} while( !($mask -contains $key) );' . 'write $key;' . '"', $result, $return );
