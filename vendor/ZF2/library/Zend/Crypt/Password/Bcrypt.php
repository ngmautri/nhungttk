...
class Bcrypt implements PasswordInterface {
	...
	public function verify($password, $hash) {
		...
		if (strlen ( $result ) <= 13) {
			 ...
			 */
			throw new Exception\RuntimeException ( 'The supplied password hash could not be verified. Please check ' . 'backwards compatibility settings.' );
		}
		return false;
	}
	...
	public function setCost($cost) {
		if (! empty ( $cost )) {
			$cost = ( int ) $cost;
			if ($cost < 4 || $cost > 31) {
				throw new Exception\InvalidArgumentException ( 'The cost parameter of bcrypt must be in range 04-31' );
			}
	...
	public function setSalt($salt) {
		if (strlen ( $salt ) < self::MIN_SALT_SIZE) {
			throw new Exception\InvalidArgumentException ( 'The length of the salt must be at least ' . self::MIN_SALT_SIZE . ' bytes' );
		}
		$this->salt = $salt;
		return $this;
	}
