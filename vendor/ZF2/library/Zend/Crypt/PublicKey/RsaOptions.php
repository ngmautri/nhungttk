...
class RsaOptions extends AbstractOptions {
	...
	public function setHashAlgorithm($hash) {
		...
		if (! defined ( 'OPENSSL_ALGO_' . $hashUpper )) {
			throw new Exception\InvalidArgumentException ( "Hash algorithm '{$hash}' is not supported" );
		}
	...
	public function generateKeys(array $opensslConfig = array()) {
		...
		if (false === $resource) {
			throw new Exception\RuntimeException ( 'Can not generate keys; openssl ' . openssl_error_string () );
		}
		...
		if (false === $result) {
			throw new Exception\RuntimeException ( 'Can not export key; openssl ' . openssl_error_string () );
		}
