...
class Mcrypt implements SymmetricInterface {
	...
	public function __construct($options = array()) {
		if (! extension_loaded ( 'mcrypt' )) {
			throw new Exception\RuntimeException ( 'You cannot use ' . __CLASS__ . ' without the Mcrypt extension' );
		}
		if (! empty ( $options )) {
			if ($options instanceof Traversable) {
				$options = ArrayUtils::iteratorToArray ( $options );
			} elseif (! is_array ( $options )) {
				throw new Exception\InvalidArgumentException ( 'The options parameter must be an array, a Zend\Config\Config object or a Traversable' );
			}
	...
	public function setKey($key) {
		...
		if (strlen ( $key ) < $this->getKeySize ()) {
			throw new Exception\InvalidArgumentException ( 'The size of the key must be at least of ' . $this->getKeySize () . ' bytes' );
		}
		$this->key = $key;
	...
	public function setAlgorithm($algo) {
		if (! array_key_exists ( $algo, $this->supportedAlgos )) {
			throw new Exception\InvalidArgumentException ( "The algorithm $algo is not supported by " . __CLASS__ );
		}
		$this->algo = $algo;
	...
	public function setSalt($salt) {
		...
		if (strlen ( $salt ) < $this->getSaltSize ()) {
			throw new Exception\InvalidArgumentException ( 'The size of the salt (IV) must be at least ' . $this->getSaltSize () . ' bytes' );
		}
		$this->iv = $salt;
	...
	public function getSalt() {
		...
		if (strlen ( $this->iv ) < $this->getSaltSize ()) {
			throw new Exception\RuntimeException ( 'The size of the salt (IV) must be at least ' . $this->getSaltSize () . ' bytes' );
		}
	...
	public function setMode($mode) {
		if (! empty ( $mode )) {
			...
			if (! array_key_exists ( $mode, $this->supportedModes )) {
				throw new Exception\InvalidArgumentException ( "The mode $mode is not supported by " . __CLASS__ );
			}
			$this->mode = $mode;
		}
