...
class Connection implements ConnectionInterface, Profiler\ProfilerAwareInterface {
	...
	public function __construct($connectionParameters = null) {
		if (is_array ( $connectionParameters )) {
			...
			throw new Exception\InvalidArgumentException ( '$connection must be an array of parameters, a db2 connection resource or null' );
		}
	}
	...
	public function connect() {
		...
		$connection = array ();
		$connection ['database'] = $findParameterValue ( array (
				'database',
				'db' 
		) );
		$connection ['username'] = $findParameterValue ( array (
				'username',
				'uid',
				'UID' 
		) );
