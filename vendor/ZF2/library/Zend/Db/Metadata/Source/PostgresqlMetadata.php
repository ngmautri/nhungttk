...
class PostgresqlMetadata extends AbstractSource {
	protected function loadSchemaData() {
		...
		$sql = 'SELECT ' . $p->quoteIdentifier ( 'schema_name' ) . ' FROM ' . $p->quoteIdentifierChain ( array (
				'information_schema',
				'schemata' 
		) ) . ' WHERE ' . $p->quoteIdentifier ( 'schema_name' ) . ' != \'information_schema\'' . ' AND ' . $p->quoteIdentifier ( 'schema_name' ) . " NOT LIKE 'pg_%'";
	...
	protected function loadTableNameData($schema) {
		...
		$sql = 'SELECT ' . implode ( ', ', $isColumns ) . ' FROM ' . $p->quoteIdentifierChain ( array (
				'information_schema',
				'tables' 
		) ) . ' t' . 
		...
		' LEFT JOIN ' . $p->quoteIdentifierChain ( array (
				'information_schema',
				'views' 
		) ) . ' v' . ' ON ' . $p->quoteIdentifierChain ( array (
				't',
				'table_schema' 
		) ) . '  = ' . $p->quoteIdentifierChain ( array (
				'v',
				'table_schema' 
		...
		' WHERE ' . $p->quoteIdentifierChain ( array (
				't',
				'table_type' 
		) ) . ' IN (\'BASE TABLE\', \'VIEW\')';
		
		if ($schema != self::DEFAULT_SCHEMA) {
			$sql .= ' AND ' . $p->quoteIdentifierChain ( array (
					't',
					'table_schema' 
	...
	protected function loadColumnData($table, $schema) {
		...
		$sql = 'SELECT ' . implode ( ', ', $isColumns ) . ' FROM ' . $platform->quoteIdentifier ( 'information_schema' ) . $platform->getIdentifierSeparator () . $platform->quoteIdentifier ( 'columns' ) . ' WHERE ' . $platform->quoteIdentifier ( 'table_schema' ) . ' != \'information\'' . ' AND ' . $platform->quoteIdentifier ( 'table_name' ) . ' = ' . $platform->quoteTrustedValue ( $table );
	...
	protected function loadConstraintData($table, $schema) {
		...
		$isColumns = array (
				...
				array (
						'referenced_table_schema' => 'kcu2',
						'table_schema' 
				),
				array (
						'referenced_table_name' => 'kcu2',
						'table_name' 
				),
				array (
						'referenced_column_name' => 'kcu2',
						'column_name' 
				) 
		);
		...
		' INNER JOIN ' . $p->quoteIdentifierChain ( array (
				'information_schema',
				'table_constraints' 
		) ) . ' tc' . ' ON ' . $p->quoteIdentifierChain ( array (
				't',
				'table_schema' 
		) ) . '  = ' . $p->quoteIdentifierChain ( array (
				'tc',
		...
		' LEFT JOIN ' . $p->quoteIdentifierChain ( array (
				'information_schema',
				'key_column_usage' 
		) ) . ' kcu' . ' ON ' . $p->quoteIdentifierChain ( array (
				'tc',
				'table_schema' 
		) ) . '  = ' . $p->quoteIdentifierChain ( array (
				'kcu',
		...
		' LEFT JOIN ' . $p->quoteIdentifierChain ( array (
				'information_schema',
				'check_constraints' 
		) ) . ' cc' . ' ON ' . $p->quoteIdentifierChain ( array (
				'tc',
				'constraint_schema' 
		) ) . '  = ' . $p->quoteIdentifierChain ( array (
				'cc',
		...
		) ) . ' rc' . ' ON ' . $p->quoteIdentifierChain ( array (
				'tc',
				'constraint_schema' 
		) ) . '  = ' . $p->quoteIdentifierChain ( array (
				'rc',
		...
		' LEFT JOIN ' . $p->quoteIdentifierChain ( array (
				'information_schema',
				'key_column_usage' 
		) ) . ' kcu2' . ' ON ' . $p->quoteIdentifierChain ( array (
				'rc',
		...
		' WHERE ' . $p->quoteIdentifierChain ( array (
				't',
				'table_name' 
		) ) . ' = ' . $p->quoteTrustedValue ( $table ) . ' AND ' . $p->quoteIdentifierChain ( array (
				't',
				'table_type' 
		) ) . ' IN (\'BASE TABLE\', \'VIEW\')';
		...
		$sql .= ' ORDER BY CASE ' . $p->quoteIdentifierChain ( array (
				'tc',
				'constraint_type' 
		) ) . " WHEN 'PRIMARY KEY' THEN 1" . " WHEN 'UNIQUE' THEN 2" . " WHEN 'FOREIGN KEY' THEN 3" . " WHEN 'CHECK' THEN 4" . " ELSE 5 END" . ', ' . $p->quoteIdentifierChain ( array (
				'tc',
				'constraint_name' 
		) ) . ', ' . $p->quoteIdentifierChain ( array (
				'kcu',
	...
	protected function loadTriggerData($schema) {
		...
		if ($schema != self::DEFAULT_SCHEMA) {
			...
			$sql .= $p->quoteIdentifier ( 'trigger_schema' ) . ' != \'information_schema\'';
		}
