...
class Di implements DependencyInjectionInterface {
	...
	public function get($name, array $params = array()) {
		array_push ( $this->instanceContext, array (
				'GET',
				$name,
				null 
		) );
										...
										if ($methodParams) {
											foreach ( $methodParams as $methodParam ) {
												...
												if ($objectToInjectClass == $methodParam [1] || self::isSubclassOf ( $objectToInjectClass, $methodParam [1] )) {
													if ($this->resolveAndCallInjectionMethodForInstance ( $instance, $typeInjectionMethod, array (
															$methodParam [0] => $objectToInject 
													), $instanceAlias, self::METHOD_IS_REQUIRED, $type )) {
														$calledMethods [$typeInjectionMethod] = true;
													}
													continue 3;
												}
	...
	protected function resolveMethodParameters($class, $method, array $callTimeUserParams, $alias, $methodRequirementType, $isInstantiator = false) {
		...
		if (isset ( $this->instanceContext [0] )) {
			list ( $requestedClass, $requestedAlias ) = ($this->instanceContext [0] [0] == 'NEW') ? array (
					$this->instanceContext [0] [1],
					$this->instanceContext [0] [2] 
			) : array (
					$this->instanceContext [1] [1],
