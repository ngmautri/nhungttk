...
class EventManager implements EventManagerInterface {
	 ...
	 *
	 * @param string|array|ListenerAggregateInterface $event
	...
	public function attach($event, $callback = null, $priority = 1) {
		...
		if (null === $callback) {
			throw new Exception\InvalidArgumentException ( sprintf ( '%s: expects a callback; none provided', __METHOD__ ) );
		}
	...
	public function detach($listener) {
		...
		if (! $listener instanceof CallbackHandler) {
			throw new Exception\InvalidArgumentException ( sprintf ( '%s: expected a ListenerAggregateInterface or CallbackHandler; received "%s"', __METHOD__, (is_object ( $listener ) ? get_class ( $listener ) : gettype ( $listener )) ) );
		}
