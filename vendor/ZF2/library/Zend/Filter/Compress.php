...
class Compress extends AbstractFilter {
	...
	public function setOptions($options) {
		if (! is_array ( $options ) && ! $options instanceof Traversable) {
			throw new Exception\InvalidArgumentException ( sprintf ( '"%s" expects an array or Traversable; received "%s"', __METHOD__, (is_object ( $options ) ? get_class ( $options ) : gettype ( $options )) ) );
		}
	...
	public function getAdapter() {
		...
		if (! class_exists ( $adapter )) {
			...
			if (! class_exists ( $adapter )) {
				throw new Exception\RuntimeException ( sprintf ( '%s unable to load adapter; class "%s" not found', __METHOD__, $this->adapter ) );
			}
		}
	 ...
	 *
	 * @param string|Compress\CompressionAlgorithmInterface $adapter
	 *        	Adapter to use
	 * @return Compress
	 * @throws Exception\InvalidArgumentException
	 */
