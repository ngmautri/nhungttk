...
class RenameUpload extends AbstractFilter {
	...
	public function setTarget($target) {
		if (! is_string ( $target )) {
			throw new Exception\InvalidArgumentException ( 'Invalid target, must be a string' );
		}
	 ...
	 *
	 * @param bool $flag
	 *        	When true, this filter will use the original file
	...
	protected function checkFileExists($targetFile) {
		if (file_exists ( $targetFile )) {
			if ($this->getOverwrite ()) {
				...
				throw new Exception\InvalidArgumentException ( sprintf ( "File '%s' could not be renamed. It already exists.", $targetFile ) );
			}
		}
	}
