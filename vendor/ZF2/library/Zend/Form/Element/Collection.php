...
class Collection extends Fieldset implements FieldsetPrepareAwareInterface {
	...
	public function setObject($object) {
		if (! is_array ( $object ) && ! $object instanceof Traversable) {
			throw new Exception\InvalidArgumentException ( sprintf ( '%s expects an array or Traversable object argument; received "%s"', __METHOD__, (is_object ( $object ) ? get_class ( $object ) : gettype ( $object )) ) );
		}
	...
	public function populateValues($data) {
		...
		if (! empty ( $data ) && $this->allowAdd) {
			...
			throw new Exception\DomainException ( sprintf ( 'There are more elements than specified in the collection (%s). Either set the allow_add option ' . 'to true, or re-submit the form.', get_class ( $this ) ) );
		}
