...
abstract class AbstractWord extends FormInput {
	...
	protected function renderCaptchaHidden(CaptchaAdapter $captcha, array $attributes) {
		...
		$hidden = sprintf ( '<input %s%s', $this->createAttributesString ( $attributes ), $closingBracket );
	...
	public function setCaptchaPosition($captchaPosition) {
		...
		if (! in_array ( $captchaPosition, array (
			...
			throw new Exception\InvalidArgumentException ( sprintf ( '%s expects either %s::CAPTCHA_APPEND or %s::CAPTCHA_PREPEND; received "%s"', __METHOD__, __CLASS__, __CLASS__, ( string ) $captchaPosition ) );
		}
