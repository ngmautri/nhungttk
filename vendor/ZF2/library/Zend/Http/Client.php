...
class Client implements Stdlib\DispatchableInterface {
	...
	public function setMethod($method) {
		...
		if (($method == Request::METHOD_POST || $method == Request::METHOD_PUT || $method == Request::METHOD_DELETE || $method == Request::METHOD_PATCH) && empty ( $this->encType )) {
	 ...
	 *        	Also clear http authentication? (defaults to true)
	 * @return Client
	 */
		...
		switch (strtolower ( $type )) {
			...
			case self::AUTH_DIGEST :
				...
				if (empty ( $digest ['qop'] )) {
					...
					$response = md5 ( $ha1 . ':' . $digest ['nonce'] . ':' . $digest ['nc'] . ':' . $digest ['cnonce'] . ':' . $digest ['qoc'] . ':' . $ha2 );
				}
				break;
		}
		return $response;
