...
class Curl implements HttpAdapter, StreamInterface {
	...
	public function setOptions($options = array()) {
		...
		if (! is_array ( $options )) {
			throw new AdapterException\InvalidArgumentException ( 'Array or Traversable object expected, got ' . gettype ( $options ) );
		}
	...
	public function connect($host, $port = 80, $secure = false) {
		...
		if ($this->curl && is_array ( $this->connectedTo ) && ($this->connectedTo [0] != $host || $this->connectedTo [1] != $port)) {
			$this->close ();
		}
