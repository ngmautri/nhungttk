...
class Proxy extends Socket {
	...
	public function connect($host, $port = 80, $secure = false) {
		...
		parent::connect ( $this->config ['proxy_host'], $this->config ['proxy_port'], false );
	}
	...
	public function write($method, $uri, $httpVer = '1.1', $headers = array(), $body = '') {
		...
		if ($this->config ['proxy_user'] && ! isset ( $headers ['proxy-authorization'] )) {
			$headers ['proxy-authorization'] = Client::encodeAuthHeader ( $this->config ['proxy_user'], $this->config ['proxy_pass'], $this->config ['proxy_auth'] );
		}
	...
	protected function connectHandshake($host, $port = 443, $httpVer = '1.1', array &$headers = array()) {
		...
		if (! $success) {
			throw new AdapterException\RuntimeException ( "Unable to connect to" . " HTTPS server through proxy: could not negotiate secure connection." );
		}
	}
