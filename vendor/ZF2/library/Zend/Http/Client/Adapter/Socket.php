...
class Socket implements HttpAdapter, StreamInterface {
	...
	public function setOptions($options = array()) {
		...
		if (! is_array ( $options )) {
			throw new AdapterException\InvalidArgumentException ( 'Array or Zend\Config object expected, got ' . gettype ( $options ) );
		}
	...
	public function setStreamContext($context) {
		if (is_resource ( $context ) && get_resource_type ( $context ) == 'stream-context') {
			...
			// Invalid parameter
			throw new AdapterException\InvalidArgumentException ( "Expecting either a stream context resource or array, got " . gettype ( $context ) );
		}
	...
	public function connect($host, $port = 80, $secure = false) {
		...
		$connectedHost = (strpos ( $this->connectedTo [0], '://' )) ? substr ( $this->connectedTo [0], (strpos ( $this->connectedTo [0], '://' ) + 3), strlen ( $this->connectedTo [0] ) ) : $this->connectedTo [0];
		...
		if (! is_resource ( $this->socket ) || ! $this->config ['keepalive']) {
			...
			if (! $this->socket) {
				$this->close ();
				throw new AdapterException\RuntimeException ( sprintf ( 'Unable to connect to %s:%d%s', $host, $port, ($error ? ' . Error #' . $error->getCode () . ': ' . $error->getMessage () : '') ), 0, $error );
			}
			...
			if ($secure || $this->config ['sslusecontext']) {
				...
				if (! $test || $error) {
					...
					throw new AdapterException\RuntimeException ( sprintf ( 'Unable to enable crypto on TCP connection %s%s', $host, $errorString ), 0, $error );
				}
	...
	public function read() {
		 ...
		 */
		if ($statusCode == 304 || $statusCode == 204 || $this->method == \Zend\Http\Request::METHOD_HEAD) {
		...
		if ($transferEncoding !== false) {
			...
			if (strtolower ( $transferEncoding->getFieldValue () ) == 'chunked') {
				...
				do {
					...
					if (! ctype_xdigit ( $chunksize )) {
						$this->close ();
						throw new AdapterException\RuntimeException ( 'Invalid chunk size "' . $chunksize . '" unable to read chunked body' );
					}
				...
				$this->close ();
				throw new AdapterException\RuntimeException ( 'Cannot handle "' . $transferEncoding->getFieldValue () . '" transfer encoding' );
			}
			...
			for($readTo = $currentPos + $contentLength; $readTo > $currentPos; $currentPos = ftell ( $this->socket )) {
	...
	protected function _checkSocketReadTimeout() {
		if ($this->socket) {
			...
			if ($timedout) {
				$this->close ();
				throw new AdapterException\TimeoutException ( "Read timed out after {$this->config['timeout']} seconds", AdapterException\TimeoutException::READ_TIMEOUT );
			}
		}
	}
