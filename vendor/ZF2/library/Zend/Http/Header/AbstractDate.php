 ...
 * @see Zend\Http\Header\LastModified Note for 'Location' header:
...
abstract class AbstractDate implements HeaderInterface {
	...
	public static function setDateFormat($format) {
		if (! isset ( static::$dateFormats [$format] )) {
			throw new Exception\InvalidArgumentException ( "No constant defined for provided date format: {$format}" );
		}
	...
	public function setDate($date) {
		if (is_string ( $date )) {
			try {
				...
				throw new Exception\InvalidArgumentException ( sprintf ( 'Invalid date passed as string (%s)', ( string ) $date ), $e->getCode (), $e );
			}
