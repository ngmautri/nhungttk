		...
		foreach ( $headers as $header ) {
			if ($header instanceof \Zend\Http\Header\ContentLength) {
				...
				if (strlen ( $response->content ) > $contentLength) {
					throw new Exception\OutOfRangeException ( sprintf ( 'Too much content was extracted from the stream (%d instead of %d bytes)', strlen ( $response->content ), $contentLength ) );
				}
				break;
			}
		}
