...
class DefaultIterator implements \Iterator, \Countable {
	...
	public function setAttributeNameTreatment($attributeNameTreatment) {
		if (is_callable ( $attributeNameTreatment )) {
			...
			} elseif (is_array ( $attributeNameTreatment ) && ! method_exists ( $attributeNameTreatment [0], $attributeNameTreatment [1] )) {
				$this->attributeNameTreatment = self::ATTRIBUTE_TO_LOWER;
			} else {
	...
	public function current() {
		...
		while ( $name ) {
			...
			ErrorHandler::start ();
			$name = ldap_next_attribute ( $resource, $this->current, $berIdentifier );
			ErrorHandler::stop ();
		}
