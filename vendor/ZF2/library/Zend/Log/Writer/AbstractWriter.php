...
abstract class AbstractWriter implements WriterInterface {
	...
	public function addFilter($filter, array $options = null) {
		...
		if (! $filter instanceof Filter\FilterInterface) {
			throw new Exception\InvalidArgumentException ( sprintf ( 'Filter must implement %s\Filter\FilterInterface; received "%s"', __NAMESPACE__, is_object ( $filter ) ? get_class ( $filter ) : gettype ( $filter ) ) );
		}
	...
	public function setFormatterPluginManager($plugins) {
		...
		if (! $plugins instanceof FormatterPluginManager) {
			throw new Exception\InvalidArgumentException ( sprintf ( 'Writer plugin manager must extend %s\FormatterPluginManager; received %s', __NAMESPACE__, is_object ( $plugins ) ? get_class ( $plugins ) : gettype ( $plugins ) ) );
		}
	...
	public function setFormatter($formatter, array $options = null) {
		...
		if (! $formatter instanceof Formatter\FormatterInterface) {
			throw new Exception\InvalidArgumentException ( sprintf ( 'Formatter must implement %s\Formatter\FormatterInterface; received "%s"', __NAMESPACE__, is_object ( $formatter ) ? get_class ( $formatter ) : gettype ( $formatter ) ) );
		}
