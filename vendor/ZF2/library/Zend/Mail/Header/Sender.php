...
class Sender implements HeaderInterface {
	...
	public function setAddress($emailOrAddress, $name = null) {
		if (is_string ( $emailOrAddress )) {
			...
			throw new Exception\InvalidArgumentException ( sprintf ( '%s expects a string or AddressInterface object; received "%s"', __METHOD__, (is_object ( $emailOrAddress ) ? get_class ( $emailOrAddress ) : gettype ( $emailOrAddress )) ) );
		}
