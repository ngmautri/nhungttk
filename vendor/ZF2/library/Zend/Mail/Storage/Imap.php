...
class Imap extends AbstractStorage implements Folder\FolderInterface, Writable\WritableInterface {
	...
	public function countMessages($flags = null) {
		...
		if ($flags === null) {
			return count ( $this->protocol->search ( array (
					'ALL' 
			) ) );
		}
	...
	public function getMessage($id) {
		$data = $this->protocol->fetch ( array (
				'FLAGS',
				'RFC822.HEADER' 
		), $id );
	 ...
	 * If you're doing that from a web environment
	...
	public function removeMessage($id) {
		if (! $this->protocol->store ( array (
				Mail\Storage::FLAG_DELETED 
		), $id, null, '+' )) {
	 ...
	 * @param string|\Zend\Mail\Storage\Folder $parentFolder
	 ...
	 * remove a folder
	 *
	 * @param string|\Zend\Mail\Storage\Folder $name
	 ...
	 *
	 * @param string|\Zend\Mail\Storage\Folder $oldName
	 *        	name or instance of folder
	 ...
	 * @param null|string|\Zend\Mail\Storage\Folder $folder
	 ...
	 * @param int $id
	 *        	number of message
	 * @param string|\Zend\Mail\Storage\Folder $folder
