...
class Part implements RecursiveIterator, Part\PartInterface {
	...
	public function getHeader($name, $format = null) {
		...
		if ($header === false) {
			...
			if ($header === false) {
				throw new Exception\InvalidArgumentException ( "Header with Name $name or $lowerName not found" );
			}
		}
		...
		switch ($format) {
			case 'string' :
				if ($header instanceof HeaderInterface) {
					...
					$return = '';
					foreach ( $header as $h ) {
						$return .= $h->getFieldValue ( HeaderInterface::FORMAT_RAW ) . Mime\Mime::LINEEND;
					}
