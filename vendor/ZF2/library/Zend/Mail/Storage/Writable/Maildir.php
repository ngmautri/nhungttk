...
class Maildir extends Folder\Maildir implements WritableInterface {
	...
	protected function _createTmpFile($folder = 'INBOX') {
		...
		if (! $fh) {
			throw new StorageException\RuntimeException ( "tried $maxTries unique ids for a temp file, but all were taken" . ' - giving up' );
		}
	 ...
	 *        	return known data of quota and current size and message count
	 * @param bool $forceRecalc        	
