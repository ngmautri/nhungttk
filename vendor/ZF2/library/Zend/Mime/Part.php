	...
	public function getEncodedStream($EOL = Mime::LINEEND) {
		...
		switch ($this->encoding) {
			case Mime::ENCODING_QUOTEDPRINTABLE :
				$filter = stream_filter_append ( $this->content, 'convert.quoted-printable-encode', STREAM_FILTER_READ, array (
						'line-length' => 76,
			...
			case Mime::ENCODING_BASE64 :
				$filter = stream_filter_append ( $this->content, 'convert.base64-encode', STREAM_FILTER_READ, array (
						'line-length' => 76,
						'line-break-chars' => $EOL 
				) );
				if (! is_resource ( $filter )) {
