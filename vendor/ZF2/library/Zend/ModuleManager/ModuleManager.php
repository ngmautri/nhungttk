...
class ModuleManager implements ModuleManagerInterface {
		...
		foreach ( $this->getModules () as $moduleName => $module ) {
			if (is_object ( $module )) {
				if (! is_string ( $moduleName )) {
					throw new Exception\RuntimeException ( sprintf ( 'Module (%s) must have a key identifier.', get_class ( $module ) ) );
				}
				$module = array (
						$moduleName => $module 
				);
	...
	public function loadModules() {
		...
		/**
		 * Having a dedicated .
		 * post event abstracts the complexity of priorities from the user.
	...
	public function setModules($modules) {
		if (is_array ( $modules ) || $modules instanceof Traversable) {
			...
			throw new Exception\InvalidArgumentException ( sprintf ( 'Parameter to %s\'s %s method must be an array or implement the Traversable interface', __CLASS__, __METHOD__ ) );
		}
		return $this;
	}
