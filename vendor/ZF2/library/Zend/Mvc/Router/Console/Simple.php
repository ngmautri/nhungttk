...
class Simple implements RouteInterface {
	...
	public static function factory($options = array()) {
		...
		return new static ( $options ['route'], $options ['constraints'], $options ['defaults'], $options ['aliases'], $options ['filters'], $options ['validators'] );
	}
	...
	protected function parseRouteDefinition($def) {
		...
		while ( $pos < $length ) {
			 ...
			 * [--param]
			 */
			elseif (preg_match ( '/\G\[ *?--(?P<name>[a-zA-Z0-9][a-zA-Z0-9\_\-]+) *?\](?: +|$)/s', $def, $m, 0, $pos )) {
				$item = array (
						'name' => $m ['name'],
						'short' => false,
						'literal' => false,
			...
			elseif (preg_match ( '/\G(?P<name>[a-z0-9][a-zA-Z0-9\_\-]*?)(?: +|$)/s', $def, $m, 0, $pos )) {
				...
				throw new Exception\InvalidArgumentException ( 'Cannot understand Console route at "' . substr ( $def, $pos ) . '"' );
			}
