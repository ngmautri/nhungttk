...
abstract class AbstractContainer implements Countable, RecursiveIterator {
	 ...
	 *
	 * @param Page\AbstractPage|array|Traversable $page
	 *        	page to add
	 ...
	 *
	 * @param array|Traversable|AbstractContainer $pages
	 *        	pages to add
	 * @return AbstractContainer fluent interface, returns self
	...
	public function addPages($pages) {
		if (! is_array ( $pages ) && ! $pages instanceof Traversable) {
			throw new Exception\InvalidArgumentException ( 'Invalid argument: $pages must be an array, an ' . 'instance of Traversable or an instance of ' . 'Zend\Navigation\AbstractContainer' );
		}
	 ...
	 *        	[optional] whether to search recursively.
	 *        	Default is false.
	...
	public function __call($method, $arguments) {
		...
		if (! $result) {
			throw new Exception\BadMethodCallException ( sprintf ( 'Bad method call: Unknown method %s::%s', get_class ( $this ), $method ), 0, $error );
		}
	...
	public function current() {
		...
		$hash = key ( $this->index );
		if (! isset ( $this->pages [$hash] )) {
			throw new Exception\OutOfBoundsException ( 'Corruption detected in container; ' . 'invalid key found in internal iterator' );
		}
