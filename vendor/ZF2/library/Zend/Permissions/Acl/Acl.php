...
class Acl implements AclInterface {
	...
	public function addRole($role, $parents = null) {
		if (is_string ( $role )) {
			...
			throw new Exception\InvalidArgumentException ( 'addRole() expects $role to be of type Zend\Permissions\Acl\Role\RoleInterface' );
		}
	...
	public function addResource($resource, $parent = null) {
		if (is_string ( $resource )) {
			...
			throw new Exception\InvalidArgumentException ( 'addResource() expects $resource to be of type Zend\Permissions\Acl\Resource\ResourceInterface' );
		}
		...
		if (null !== $parent) {
			try {
				...
				throw new Exception\InvalidArgumentException ( sprintf ( 'Parent Resource id "%s" does not exist', $resourceParentId ), 0, $e );
			}
	 ...
	 * @return Acl Provides a fluent interface
	 */
	public function setRule($operation, $type, $roles = null, $resources = null, $privileges = null, Assertion\AssertionInterface $assert = null) {
		// ensure that the rule type is valid; normalize input to uppercase
		$type = strtoupper ( $type );
		if (self::TYPE_ALLOW !== $type && self::TYPE_DENY !== $type) {
			throw new Exception\InvalidArgumentException ( sprintf ( 'Unsupported rule type; must be either "%s" or "%s"', self::TYPE_ALLOW, self::TYPE_DENY ) );
		}
		...
		switch ($operation) {
			...
			default :
				throw new Exception\InvalidArgumentException ( sprintf ( 'Unsupported operation; must be either "%s" or "%s"', self::OP_ADD, self::OP_REMOVE ) );
		}
	 ...
	 * @throws Exception\RuntimeException
	 */
	protected function roleDFSVisitOnePrivilege(Role\RoleInterface $role, Resource\ResourceInterface $resource = null, $privilege = null, &$dfs = null) {
		if (null === $privilege) {
			/**
			 *
	...
	protected function getRuleType(Resource\ResourceInterface $resource = null, Role\RoleInterface $role = null, $privilege = null) {
		...
		if ($rule ['assert']) {
			...
			$assertionValue = $assertion->assert ( $this, ($this->isAllowedRole instanceof Role\RoleInterface) ? $this->isAllowedRole : $role, ($this->isAllowedResource instanceof Resource\ResourceInterface) ? $this->isAllowedResource : $resource, $this->isAllowedPrivilege );
		}
