...
class Registry {
	...
	public function add(RoleInterface $role, $parents = null) {
		...
		if ($this->has ( $roleId )) {
			throw new Exception\InvalidArgumentException ( sprintf ( 'Role id "%s" already exists in the registry', $roleId ) );
		}
		...
		if (null !== $parents) {
			...
			foreach ( $parents as $parent ) {
				try {
					...
					throw new Exception\InvalidArgumentException ( sprintf ( 'Parent Role id "%s" does not exist', $roleParentId ), 0, $e );
				}
