...
class ServiceManager implements ServiceLocatorInterface {
	...
	public function setShareByDefault($shareByDefault) {
		if ($this->allowOverride === false) {
			throw new Exception\RuntimeException ( sprintf ( '%s: cannot alter default shared service setting; container is marked immutable (allow_override is false)', get_class ( $this ) . '::' . __FUNCTION__ ) );
		}
	...
	public function setInvokableClass($name, $invokableClass, $shared = null) {
		...
		if ($this->has ( array (
			...
			if ($this->allowOverride === false) {
				throw new Exception\InvalidServiceNameException ( sprintf ( 'A service by the name or alias "%s" already exists and cannot be overridden; please use an alternate name', $name ) );
			}
	...
	public function setFactory($name, $factory, $shared = null) {
		...
		if (! ($factory instanceof FactoryInterface || is_string ( $factory ) || is_callable ( $factory ))) {
			throw new Exception\InvalidArgumentException ( 'Provided abstract factory must be the class name of an abstract factory or an instance of an AbstractFactoryInterface.' );
		}
	 ...
	 * @param string $delegatorFactoryName
	...
	public function setService($name, $service) {
		...
		if ($this->has ( $cName, false )) {
			if ($this->allowOverride === false) {
				throw new Exception\InvalidServiceNameException ( sprintf ( '%s: A service by the name "%s" or alias already exists and cannot be overridden, please use an alternate name.', get_class ( $this ) . '::' . __FUNCTION__, $name ) );
			}
			$this->unregisterService ( $cName );
		}
	...
	public function setShared($name, $isShared) {
		...
		if (! isset ( $this->invokableClasses [$cName] ) && ! isset ( $this->factories [$cName] ) && ! $this->canCreateFromAbstractFactory ( $cName, $name )) {
			throw new Exception\ServiceNotFoundException ( sprintf ( '%s: A service by the name "%s" was not found and could not be marked as shared', get_class ( $this ) . '::' . __FUNCTION__, $name ) );
		}
	...
	public function get($name, $usePeeringServiceManagers = true) {
		...
		if (! $instance) {
			if (isset ( $this->invokableClasses [$cName] ) || isset ( $this->factories [$cName] ) || isset ( $this->aliases [$cName] ) || isset ( $this->instances [$cName] ) || $this->canCreateFromAbstractFactory ( $cName, $name )) {
		...
		if ($instance === null) {
			...
			throw new Exception\ServiceNotFoundException ( sprintf ( '%s was unable to fetch or create an instance for %s', get_class ( $this ) . '::' . __FUNCTION__, $name ) );
		}
		
		if (($this->shareByDefault && ! isset ( $this->shared [$cName] )) || (isset ( $this->shared [$cName] ) && $this->shared [$cName] === true)) {
	...
	public function create($name) {
		...
		if (isset ( $this->delegators [$cName] )) {
			...
			for($i = 0; $i < $additionalDelegators; $i += 1) {
				$creationCallback = $this->createDelegatorCallback ( $this->delegators [$cName] [$i], $rName, $cName, $creationCallback );
			}
	 ...
	 *
	 * @param string $delegatorFactoryName
	 *        	name of the delegator factory service
	 * @param string $rName
	 *        	requested service name
	 * @param string $cName
	 *        	canonical service name
	 * @param callable $creationCallback
	 *        	callback that is responsible for instantiating the service
	 *        	
	 * @return callable
	 */
	...
	public function doCreate($rName, $cName) {
		...
		if ($instance === false && $this->throwExceptionInCreate) {
			throw new Exception\ServiceNotFoundException ( sprintf ( 'No valid instance was found for %s%s', $cName, ($rName ? '(alias: ' . $rName . ')' : '') ) );
		}
	...
	public function canCreate($name, $checkAbstractFactories = true) {
		...
		return (isset ( $this->invokableClasses [$cName] ) || isset ( $this->factories [$cName] ) || isset ( $this->aliases [$cName] ) || isset ( $this->instances [$cName] ) || ($checkAbstractFactories && $this->canCreateFromAbstractFactory ( $cName, $rName )));
	}
	...
	public function has($name, $checkAbstractFactories = true, $usePeeringServiceManagers = true) {
		...
		if (isset ( $this->invokableClasses [$cName] ) || isset ( $this->factories [$cName] ) || isset ( $this->aliases [$cName] ) || isset ( $this->instances [$cName] ) || ($checkAbstractFactories && $this->canCreateFromAbstractFactory ( $cName, $name ))) {
			return true;
		}
	...
	public function canCreateFromAbstractFactory($cName, $rName) {
		...
		foreach ( $this->abstractFactories as $abstractFactory ) {
			...
			if (isset ( $this->pendingAbstractFactoryRequests [$factoryClass] ) && $this->pendingAbstractFactoryRequests [$factoryClass] == $rName) {
				return false;
			}
	...
	public function setAlias($alias, $nameOrAlias) {
		...
		if ($this->allowOverride === false && $this->has ( array (
			...
			throw new Exception\InvalidServiceNameException ( sprintf ( 'An alias by the name "%s" or "%s" already exists', $cAlias, $alias ) );
		}
	...
	protected function createServiceViaCallback($callable, $cName, $rName) {
		...
		try {
			...
			unset ( $circularDependencyResolver [$depKey] );
			throw new Exception\ServiceNotCreatedException ( sprintf ( 'An exception was raised while creating "%s"; no instance returned', $rName ), $e->getCode (), $e );
		}
		if ($instance === null) {
	...
	protected function createFromFactory($canonicalName, $requestedName) {
		...
		if ($factory instanceof FactoryInterface) {
			...
			throw new Exception\ServiceNotCreatedException ( sprintf ( 'While attempting to create %s%s an invalid factory was registered for this instance type.', $canonicalName, ($requestedName ? '(alias: ' . $requestedName . ')' : '') ) );
		}
		return $instance;
	}
	...
	protected function createFromAbstractFactory($canonicalName, $requestedName) {
		foreach ( $this->abstractFactories as $index => $abstractFactory ) {
			...
			if (is_string ( $abstractFactory ) && class_exists ( $abstractFactory, true )) {
				...
				throw new Exception\ServiceNotCreatedException ( sprintf ( 'While attempting to create %s%s an abstract factory could not produce a valid instance.', $canonicalName, ($requestedName ? '(alias: ' . $requestedName . ')' : '') ) );
			}
			try {
				...
				unset ( $this->pendingAbstractFactoryRequests [get_class ( $abstractFactory )] );
				throw new Exception\ServiceNotCreatedException ( sprintf ( 'An abstract factory could not create an instance of %s%s.', $canonicalName, ($requestedName ? '(alias: ' . $requestedName . ')' : '') ), $e->getCode (), $e );
			}
