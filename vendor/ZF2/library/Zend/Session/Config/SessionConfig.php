...
class SessionConfig extends StandardConfig {
	...
	public function setStorageOption($storageName, $storageValue) {
		...
		if (FALSE === $result) {
			throw new \InvalidArgumentException ( "'" . $key . "' is not a valid sessions-related ini setting." );
		}
		return $this;
	}
	...
	public function setPhpSaveHandler($phpSaveHandler) {
		...
		if ($this->phpErrorCode >= E_WARNING) {
			throw new Exception\InvalidArgumentException ( 'Invalid save handler specified: ' . $this->phpErrorMessage );
		}
	...
	public function setHashBitsPerCharacter($hashBitsPerCharacter) {
		if (! is_numeric ( $hashBitsPerCharacter ) || ! in_array ( $hashBitsPerCharacter, $this->validHashBitsPerCharacters )) {
