...
class StandardConfig implements ConfigInterface {
	...
	public function setEntropyFile($entropyFile) {
		if (! is_readable ( $entropyFile )) {
			throw new Exception\InvalidArgumentException ( sprintf ( "Invalid entropy_file provided: '%s'; doesn't exist or not readable", $entropyFile ) );
		}
	...
	public function __call($method, $args) {
		...
		if ($prefix === 'set') {
			...
			throw new Exception\BadMethodCallException ( sprintf ( 'Method "%s" does not exist in %s', $method, get_class ( $this ) ) );
		}
	}
}
