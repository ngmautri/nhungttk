...
class MongoDB implements SaveHandlerInterface {
	...
	public function read($id) {
		...
		if (null !== $session) {
			if ($session [$this->options->getModifiedField ()] instanceof MongoDate && $session [$this->options->getModifiedField ()]->sec + $session [$this->options->getLifetimeField ()] > time ()) {
	...
	public function write($id, $data) {
		$saveOptions = array_replace ( $this->options->getSaveOptions (), array (
				'upsert' => true,
				'multiple' => false 
		) );
	...
	public function gc($maxlifetime) {
		 ...
		 */
		$result = $this->mongoCollection->remove ( array (
				$this->options->getModifiedField () => array (
						'$lt' => new MongoDate ( time () - $maxlifetime ) 
				) 
		), $this->options->getSaveOptions () );
