...
class SessionManager extends AbstractManager {
	 ...
	 *        	If set to true, current session storage will not be overwritten by the
	...
	public function isValid() {
		...
		$responses = $validator->triggerUntil ( 'session.validate', $this, array (
				$this 
		), function ($test) {
			return ! $test;
		} );
	...
	public function expireSessionCookie() {
		...
		setcookie ( $this->getName (), // session name
'', // value
$_SERVER ['REQUEST_TIME'] - 42000, // TTL for cookie
$config->getCookiePath (), $config->getCookieDomain (), $config->getCookieSecure (), $config->getCookieHttpOnly () );
	}
