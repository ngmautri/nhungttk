...
abstract class AbstractHttpControllerTestCase extends AbstractControllerTestCase {
	...
	public function assertHasResponseHeader($header) {
		...
		if (false === $responseHeader) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting response header "%s" found', $header ) );
		}
	...
	public function assertNotHasResponseHeader($header) {
		$responseHeader = $this->getResponseHeader ( $header );
		if (false !== $responseHeader) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting response header "%s" WAS NOT found', $header ) );
		}
	...
	public function assertResponseHeaderContains($header, $match) {
		$responseHeader = $this->getResponseHeader ( $header );
		if (! $responseHeader) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting response header, header "%s" do not exists', $header ) );
		}
		if ($match != $responseHeader->getFieldValue ()) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting response header "%s" exists and contains "%s", actual content is "%s"', $header, $match, $responseHeader->getFieldValue () ) );
		}
	...
	public function assertNotResponseHeaderContains($header, $match) {
		...
		if ($match == $responseHeader->getFieldValue ()) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting response header "%s" DOES NOT CONTAIN "%s"', $header, $match ) );
		}
	...
	public function assertResponseHeaderRegex($header, $pattern) {
		...
		if (! preg_match ( $pattern, $responseHeader->getFieldValue () )) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting response header "%s" exists and matches regex "%s", actual content is "%s"', $header, $pattern, $responseHeader->getFieldValue () ) );
		}
	...
	public function assertNotResponseHeaderRegex($header, $pattern) {
		...
		if (preg_match ( $pattern, $responseHeader->getFieldValue () )) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting response header "%s" DOES NOT MATCH regex "%s"', $header, $pattern ) );
		}
	...
	public function assertRedirect() {
		...
		if (false === $responseHeader) {
			throw new PHPUnit_Framework_ExpectationFailedException ( 'Failed asserting response is a redirect' );
		}
		$this->assertNotEquals ( false, $responseHeader );
	}
	...
	public function assertNotRedirect() {
		$responseHeader = $this->getResponseHeader ( 'Location' );
		if (false !== $responseHeader) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting response is NOT a redirect, actual redirection is "%s"', $responseHeader->getFieldValue () ) );
		}
		$this->assertFalse ( $responseHeader );
	}
	...
	public function assertRedirectTo($url) {
		...
		if ($url != $responseHeader->getFieldValue ()) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting response redirects to "%s", actual redirection is "%s"', $url, $responseHeader->getFieldValue () ) );
		}
	...
	public function assertNotRedirectTo($url) {
		...
		if ($url == $responseHeader->getFieldValue ()) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting response redirects to "%s"', $url ) );
		}
	...
	public function assertRedirectRegex($pattern) {
		...
		if (! preg_match ( $pattern, $responseHeader->getFieldValue () )) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting response redirects to URL MATCHING "%s", actual redirection is "%s"', $pattern, $responseHeader->getFieldValue () ) );
		}
		$this->assertTrue ( ( bool ) preg_match ( $pattern, $responseHeader->getFieldValue () ) );
	}
	...
	public function assertNotRedirectRegex($pattern) {
		...
		if (preg_match ( $pattern, $responseHeader->getFieldValue () )) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting response DOES NOT redirect to URL MATCHING "%s"', $pattern ) );
		}
		$this->assertFalse ( ( bool ) preg_match ( $pattern, $responseHeader->getFieldValue () ) );
	}
	...
	private function queryCountAssertion($path, $count, $useXpath = false) {
		...
		$match = $this->$method ( $path );
		if ($match != $count) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting node DENOTED BY %s OCCURS EXACTLY %d times, actually occurs %d times', $path, $count, $match ) );
		}
	...
	private function notQueryCountAssertion($path, $count, $useXpath = false) {
		...
		$match = $this->$method ( $path );
		if ($match == $count) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting node DENOTED BY %s DOES NOT OCCUR EXACTLY %d times', $path, $count ) );
		}
	...
	private function queryCountMinAssertion($path, $count, $useXpath = false) {
		...
		$match = $this->$method ( $path );
		if ($match < $count) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting node DENOTED BY %s OCCURS AT LEAST %d times, actually occurs %d times', $path, $count, $match ) );
		}
	...
	private function queryCountMaxAssertion($path, $count, $useXpath = false) {
		...
		$match = $this->$method ( $path );
		if ($match > $count) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting node DENOTED BY %s OCCURS AT MOST %d times, actually occurs %d times', $path, $count, $match ) );
		}
		$this->assertTrue ( $match <= $count );
	}
	...
	private function queryContentContainsAssertion($path, $match, $useXpath = false) {
		...
		if ($result->current ()->nodeValue != $match) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting node denoted by %s CONTAINS content "%s", actual content is "%s"', $path, $match, $result->current ()->nodeValue ) );
		}
	...
	private function notQueryContentContainsAssertion($path, $match, $useXpath = false) {
		...
		if ($result->current ()->nodeValue == $match) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting node DENOTED BY %s DOES NOT CONTAIN content "%s"', $path, $match ) );
		}
	...
	private function queryContentRegexAssertion($path, $pattern, $useXpath = false) {
		...
		if (! preg_match ( $pattern, $result->current ()->nodeValue )) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting node denoted by %s CONTAINS content MATCHING "%s", actual content is "%s"', $path, $pattern, $result->current ()->nodeValue ) );
		}
	...
	private function notQueryContentRegexAssertion($path, $pattern, $useXpath = false) {
		...
		if (preg_match ( $pattern, $result->current ()->nodeValue )) {
			throw new PHPUnit_Framework_ExpectationFailedException ( sprintf ( 'Failed asserting node DENOTED BY %s DOES NOT CONTAIN content MATCHING "%s"', $path, $pattern ) );
		}
