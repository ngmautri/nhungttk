...
class Figlet {
	...
	protected function _putString($string) {
		...
		if ($this->outputWidth > 1) {
			...
			if ($this->justification > 0) {
				for($i = 1; ((3 - $this->justification) * $i + $length + $this->justification - 2) < $this->outputWidth; $i ++) {
					$this->output .= ' ';
				}
			}
		}
	...
	protected function _addChar($char) {
		...
		if (($this->outlineLength + $this->currentCharWidth - $smushAmount) > $this->outlineLengthLimit || ($this->inCharLineLength + 1) > $this->inCharLineLengthLimit) {
			return false;
		}
	...
	protected function _loadFont($fontFile) {
		...
		// Get the header
		$numsRead = sscanf ( fgets ( $fp, 1000 ), '%*c%c %d %*d %d %d %d %d %d', $this->hardBlank, $this->charHeight, $this->maxLength, $smush, $cmtLines, $rightToLeft, $this->fontSmush );
		...
		while ( ! feof ( $fp ) ) {
			...
			} elseif (substr ( $uniCode, 0, 1 ) === '0' and $uniCode !== '0' or substr ( $uniCode, 0, 2 ) === '-0') {
