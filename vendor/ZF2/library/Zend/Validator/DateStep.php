...
class DateStep extends Date {
	...
	protected function convertToDateTime($param) {
		...
		if (is_int ( $param )) {
			...
			if (strpos ( $this->getFormat (), 'Y-\WW' ) === 0 && preg_match ( '/^([0-9]{4})\-W([0-9]{2})/', $param, $matches )) {
				$dateObj = new DateTime ();
				$dateObj->setISODate ( $matches [1], $matches [2] );
			} else {
				$dateObj = DateTime::createFromFormat ( $this->getFormat (), $param, $this->getTimezone () );
			}
		}
	...
	public function isValid($value) {
		...
		if (5 === $partCounts ["0"]) {
			...
			// Check date units
			if (in_array ( $intervalUnit, array (
					'years',
					'months',
					'days' 
			) )) {
