...
class EmailAddress extends AbstractValidator {
	...
	/**
	 *
	 * @param Hostname $hostnameValidator
	 *        	OPTIONAL
	 ...
	 * 
	 * @see http://en.wikipedia.org/wiki/Reserved_IP_addresses As of RFC5753 (JAN 2010), the following blocks are no longer reserved:
	 *      - 128.0.0.0/16
	 *      - 191.255.0.0/16
	 *      - 223.255.255.0/24
	 * @see http://tools.ietf.org/html/rfc5735#page-6 As of RFC6598 (APR 2012), the following blocks are now reserved:
	 *      - 100.64.0.0/10
	...
	protected function validateHostnamePart() {
		$hostname = $this->getHostnameValidator ()->setTranslator ( $this->getTranslator () )->isValid ( $this->hostname );
		if (! $hostname) {
