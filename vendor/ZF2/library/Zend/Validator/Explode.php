...
class Explode extends AbstractValidator implements ValidatorPluginManagerAwareInterface {
	...
	public function isValid($value) {
		...
		if (is_array ( $value )) {
			...
			$values = (null !== $delimiter) ? explode ( $this->valueDelimiter, $value ) : array (
					$value 
			);
		} else {
			$values = array (
		...
		if (! $validator) {
			throw new Exception\RuntimeException ( sprintf ( '%s expects a validator to be set; none given', __METHOD__ ) );
		}
