...
;
	...
	public function setMagicFile($file) {
		if ($file === false) {
			...
			throw new Exception\InvalidArgumentException ( sprintf ( 'The given magicfile ("%s") could not be read', $file ) );
		} else {
			...
			if (empty ( $this->finfo )) {
				$this->finfo = null;
				throw new Exception\InvalidMagicMimeFileException ( sprintf ( 'The given magicfile ("%s") could not be used by ext/finfo', $file ), 0, $error );
			}
	 ...
	 *        	False disables usage of magic file
	 * @return MimeType Provides fluid interface
	 */
