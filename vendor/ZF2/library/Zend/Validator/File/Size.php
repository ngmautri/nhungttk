...
;
	...
	public function setMin($min) {
		...
		if (($max !== null) && ($min > $max)) {
			throw new Exception\InvalidArgumentException ( 'The minimum must be less than or equal to the maximum file' . " size, but $min > $max" );
		}
	...
	public function setMax($max) {
		...
		if (($min !== null) && ($max < $min)) {
			throw new Exception\InvalidArgumentException ( 'The maximum must be greater than or equal to the minimum file' . " size, but $max < $min" );
		}
