...
class Gravatar extends AbstractHtmlElement {
	...
	protected function getAvatarUrl() {
		$src = $this->getGravatarUrl () . '/' . ($this->emailIsHashed ? $this->getEmail () : md5 ( $this->getEmail () )) . '?s=' . $this->getImgSize () . '&d=' . $this->getDefaultImg () . '&r=' . $this->getRating ();
		return $src;
	}
	...
	public function getImgTag() {
		...
		$html = '<img' . $this->htmlAttribs ( $this->getAttribs () ) . $this->getClosingBracket ();
	...
	public function setRating($rating) {
		switch ($rating) {
			...
			case self::RATING_X :
				...
				break;
			default :
				throw new Exception\DomainException ( sprintf ( 'The rating value "%s" is not allowed', $rating ) );
		}
