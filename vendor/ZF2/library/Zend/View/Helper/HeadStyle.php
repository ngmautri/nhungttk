...
class HeadStyle extends Placeholder\Container\AbstractStandalone {
	...
	public function itemToString(stdClass $item, $indent) {
		...
		if (! empty ( $item->attributes )) {
			$enc = 'UTF-8';
			if ($this->view instanceof View\Renderer\RendererInterface && method_exists ( $this->view, 'getEncoding' )) {
	...
	public function append($value) {
		if (! $this->isValid ( $value )) {
			throw new Exception\InvalidArgumentException ( 'Invalid value passed to append; please use appendStyle()' );
		}
	...
	public function offsetSet($index, $value) {
		if (! $this->isValid ( $value )) {
			throw new Exception\InvalidArgumentException ( 'Invalid value passed to offsetSet; please use offsetSetStyle()' );
		}
	...
	public function prepend($value) {
		if (! $this->isValid ( $value )) {
			throw new Exception\InvalidArgumentException ( 'Invalid value passed to prepend; please use prependStyle()' );
		}
