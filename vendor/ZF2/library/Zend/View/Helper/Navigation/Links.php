...
class Links extends AbstractHelper {
	...
	public function findRelation(AbstractPage $page, $rel, $type) {
		if (! in_array ( $rel, array (
			...
			throw new Exception\DomainException ( sprintf ( 'Invalid argument: $rel must be "rel" or "rev"; "%s" given', $rel ) );
		}
	...
	public function searchRevSection(AbstractPage $page) {
		...
		if ($parent) {
			if ($parent instanceof AbstractPage && $this->findRoot ( $page )->hasPage ( $parent )) {
				$found = $parent;
			}
		}
