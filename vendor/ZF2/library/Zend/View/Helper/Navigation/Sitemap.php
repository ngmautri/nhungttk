...
class Sitemap extends AbstractHelper {
	...
	public function getDomSitemap(AbstractContainer $container = null) {
		...
		foreach ( $iterator as $page ) {
			...
			if ($this->getUseSitemapValidators () && ! $locValidator->isValid ( $url )) {
				throw new Exception\RuntimeException ( sprintf ( 'Encountered an invalid URL for Sitemap XML: "%s"', $url ) );
			}
		...
		if ($this->getUseSchemaValidation ()) {
			...
			if (! $test) {
				throw new Exception\RuntimeException ( sprintf ( 'Sitemap is invalid according to XML Schema at "%s"', self::SITEMAP_XSD ), 0, $error );
			}
		}
	...
	public function setServerUrl($serverUrl) {
		...
		if ($uri->isValid ()) {
			...
			throw new Exception\InvalidArgumentException ( sprintf ( 'Invalid server URL: "%s"', $serverUrl ) );
		}
