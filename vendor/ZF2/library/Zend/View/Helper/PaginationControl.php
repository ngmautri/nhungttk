...
class PaginationControl extends AbstractHelper {
	...
	public function __invoke(Paginator\Paginator $paginator = null, $scrollingStyle = null, $partial = null, $params = null) {
		...
		if (is_array ( $partial )) {
			if (count ( $partial ) != 2) {
				throw new Exception\InvalidArgumentException ( 'A view partial supplied as an array must contain two values: the filename and its module' );
			}
	 ...
	 *
	 * @param string $style
	 *        	string 'all' | 'elastic' | 'sliding' | 'jumping'
	 */
