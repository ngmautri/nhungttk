...
class FeedRenderer implements RendererInterface {
	...
	public function render($nameOrModel, $values = null) {
		if ($nameOrModel instanceof Model) {
			...
			if (! $nameOrModel instanceof FeedModel) {
				...
				$nameOrModel = new FeedModel ( $vars, array (
						'feed_type' => $type 
				) );
			}
	...
	public function setFeedType($feedType) {
		...
		if (! in_array ( $feedType, array (
				'rss',
				'atom' 
		) )) {
			throw new Exception\InvalidArgumentException ( sprintf ( '%s expects a string of either "rss" or "atom"', __METHOD__ ) );
		}
