...
class TemplateMapResolver implements IteratorAggregate, ResolverInterface {
	...
	public function add($nameOrMap, $path = null) {
		...
		if (! is_string ( $nameOrMap )) {
			throw new Exception\InvalidArgumentException ( sprintf ( '%s: expects a string, array, or Traversable for the first argument; received "%s"', __METHOD__, (is_object ( $nameOrMap ) ? get_class ( $nameOrMap ) : gettype ( $nameOrMap )) ) );
		}
