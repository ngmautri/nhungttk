...
abstract class AbstractOpenType extends Pdf\BinaryParser\Font\AbstractFont {
	...
	protected function _parseHeadTable() {
		...
		if ($magicNumber != 0x5f0f3cf5) {
			throw new Exception\UnrecognizedFontException ( sprintf ( 'Wrong magic number. Expected: 0x5f0f3cf5; actual: %x', $magicNumber ) );
		}
...
(($platformID == 1) && ($encodingID == 0))) // Mac Roman
	...
	protected function _jumpToTable($tableName) {
		if (empty ( $this->_tableDirectory [$tableName] )) { // do not allow NULL or zero
			throw new Exception\CorruptedFontException ( "Required table '$tableName' not found!", self::REQUIRED_TABLE_NOT_FOUND );
		}
