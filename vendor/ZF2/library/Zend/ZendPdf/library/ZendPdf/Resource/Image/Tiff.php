...
class Tiff extends AbstractImage {
	...
	protected function unpackBytes($type, $bytes) {
		if (! isset ( $this->_endianType )) {
			throw new Exception\CorruptedImageException ( 'The unpackBytes function can only be used after the endianness of the file is known' );
		}
		switch ($type) {
	...
	public function __construct($imageFileName) {
		...
		if ($this->_whiteIsZero === true) {
			$imageDictionary->Decode = new InternalType\ArrayObject ( array (
